...
async function handleRollback(targetVersion: number) {
  ...
  try {
    ...
    if (res.skipped) {
      ...
      message.success(
        `已回滚到 v${targetVersion}（创建为新版本 v${res.version}）`,
      )
  ...
  <div class="versions-view">
    <div class="page-header">
      ...
      <p class="page-desc">
        ...
        <code>wiki:nginx-502</code>）查询版本，回滚会创建新版本不删除历史。
      </p>
    </div>
    ...
    <n-card :bordered="true" class="search-card">
      <n-space align="center" :size="12" wrap>
        ...
        />
        <n-button type="primary" :loading="loading" @click="loadVersions">
          加载版本
        </n-button>
    ...
    <n-card :bordered="true" v-if="currentDocKey">
      <template #header>
        <n-space align="center" :size="12" wrap>
          <span>版本历史:</span>
          ...
          <n-button quaternary size="small" :loading="loading" @click="loadVersions">
            刷新
          </n-button>
        </n-space>
      </template>
          ...
          <div class="ver-row">
            <div class="ver-main">
              <n-space align="center" :size="8">
                ...
                <n-tag v-if="v.author" size="small" :bordered="false">
                  作者: {{ v.author }}
                </n-tag>
              </n-space>
              ...
              <div class="ver-meta">
                ...
                <span class="ver-checksum">checksum: <code>{{ v.checksum }}</code></span>
              </div>
            </div>
            <n-space :size="6">
              <n-button size="small" quaternary @click="openContent(v)">
                预览
              </n-button>
              <n-button size="small" quaternary type="info" @click="openDetail(v)">
                详情
              </n-button>
              <n-button
                v-if="versions.length >= 2"
                ...
                quaternary
                type="warning"
                @click="openDiff(v.version, versions[0].version === v.version ? versions[versions.length - 1].version : versions[0].version)"
              >
                对比最新
              </n-button>
              <n-popconfirm
                v-if="isAuthenticated"
                @positive-click="handleRollback(v.version)"
              >
                <template #trigger>
                ...
                回滚到 v{{ v.version }}？将创建一个新版本（内容与 v{{ v.version }} 相同），历史不会删除。
              </n-popconfirm>
            </n-space>
          </div>
        </n-card>
    ...
    >
      ...
      <template v-else-if="contentPreview">
        <n-code
          ...
          language="markdown"
          word-wrap
          style="font-size: 13px; padding: 12px; border-radius: 6px; background: var(--n-color-target, #fafafa); max-height: 60vh; overflow: auto;"
        />
      </template>
    </n-modal>
