...
class SQLParser:
    ...
    def parse(self, path: str, doc_id: str) -> ParsedDocument:
        ...
        return ParsedDocument(
            doc_id=doc_id, source_path=path, format="sql",
            checksum=checksum, title=doc_id, elements=elements,
        )
    ...
    def _parse_sql(self, text: str) -> list[ParsedElement]:
        ...
        for stmt in statements:
            ...
            # 分类
            is_ddl = any(stmt_upper.startswith(kw) for kw in
                         ("CREATE", "ALTER", "DROP", "TRUNCATE", "RENAME"))
            is_dml = any(stmt_upper.startswith(kw) for kw in
                         ("SELECT", "INSERT", "UPDATE", "DELETE", "MERGE", "REPLACE"))
            is_dcl = any(stmt_upper.startswith(kw) for kw in
                         ("GRANT", "REVOKE"))
            is_tcl = any(stmt_upper.startswith(kw) for kw in
                         ("COMMIT", "ROLLBACK", "SAVEPOINT", "BEGIN"))

            category = "ddl" if is_ddl else ("dml" if is_dml else
                       ("dcl" if is_dcl else ("tcl" if is_tcl else "other")))
    ...
    def _extract_tables(self, stmt: str, category: str) -> list[str]:
        tables: list[str] = []
        # CREATE TABLE xxx
        m = re.search(r"CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?(\w[\w.]*)", stmt, re.IGNORECASE)
