...
from app.knowledge import (
    get_wiki_compiler, make_slug,
    rebuild_index, get_index, list_wiki_pages, get_all_slugs,
    parse_wikilinks, render_wikilinks_html, render_wikilinks_text,
    get_backlinks, get_outlinks, get_orphan_slugs, get_all_deadlinks,
    detect_drift, mark_pages_stale, clear_stale, list_stale_pages,
    get_compiled_checksum, record_compiled_checksum,
    recall_pages, get_wiki_qa_engine,
    lint_all, suggest_missing_pages,
    auto_recompile_stale,
)


@app.post("/llm-wiki/ingest", dependencies=[Depends(verify_token)])
...
@app.post("/llm-wiki/recompile/{doc_id}", dependencies=[Depends(verify_token)])
...
@app.post("/llm-wiki/index/rebuild", dependencies=[Depends(verify_token)])
async def llm_wiki_index_rebuild() -> dict:
    """重建 index.md"""
    return rebuild_index()


@app.get("/llm-wiki/stale")
...
@app.post("/llm-wiki/drift/check/{doc_id}", dependencies=[Depends(verify_token)])
...
async def llm_wiki_deadlinks() -> dict:
    ...
    return {
        "count": len(dead),
        "deadlinks": [
            {"source": d.source_slug, "target": d.slug, "line": d.line}
            for d in dead
        ],
    }


@app.get("/llm-wiki/backlinks/{slug}")
...
@app.post("/llm-wiki/lint", dependencies=[Depends(verify_token)])
...
@app.get("/llm-wiki/lint/suggestions")
...
@app.post("/llm-wiki/recompile-stale", dependencies=[Depends(verify_token)])
