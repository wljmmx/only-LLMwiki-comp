...
## 输出格式（严格 JSON 数组）
[
  {"entity_type": "Service", "name": "auth-service", "properties": {"tier": "t1"}, "confidence": 0.92, "evidence_span": "原文证据"},
  {"relation_type": "RUNS_ON", "from_entity": "auth-service", "to_entity": "web-01", "confidence": 0.88, "evidence_span": "原文证据"}
]
...
class KnowledgeExtractor:
    ...
    def _build_context(self, doc: ParsedDocument) -> str:
        ...
        for e in doc.elements[:20]:
            label = f"[{e.type.value}]"
            if e.section:
                label += f"（章节：{e.section}）"
            lines.append(f"{label} {e.content[:500]}")
            lines.append("")
        return "\n".join(lines)
