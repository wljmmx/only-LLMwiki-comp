...
from app.knowledge.wikilink import (
    parse_wikilinks, get_orphan_slugs, get_all_deadlinks,
)
            ...
            "issues": [
                {
                    ...
                    "slug": i.slug, "message": i.message, "detail": i.detail,
                }
...
def lint_all(*, include_stale: bool = True) -> LintReport:
    ...
    for slug, content in page_contents.items():
        ...
        page_severity = SEV_WARN if meta.get("review_status") == "review_needed" else SEV_INFO
...
def _check_contradictions(page_contents: dict[str, str]) -> list[LintIssue]:
    ...
    for key, vals in param_values.items():
        ...
        if len(distinct_vals) < 2:
            continue
        # 数值型矛盾优先报
        numeric_vals = [v for v in unique.values() if re.match(r"^\d+(\.\d+)?\s*\w*$", v)]
