...
def compute_issue_key(issue_type: str, slug: str, message: str) -> str:
    """P1-12b: 派生 lint issue 的稳定标识

    lint issue 实时计算、无原生 ID，且 (type, slug) 不唯一（同 slug 可有多条
    missing_type_section，靠 message 区分）。用 sha1(type|slug|message)[:16]
    作为忽略表主键，保证幂等且跨调用稳定。
    """
    raw = f"{issue_type}|{slug}|{message}"
...
class LintReport:
    ...
    # P1-12b: 被忽略的 issue 数（已从 issues/by_* 中扣除）
    ignored_count: int = 0
    ...
    def recompute(self) -> None:
        """根据当前 issues 重算 by_type / by_severity 计数"""
        self.by_type = {}
        self.by_severity = {}
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "issues": [
                {
                    "issue_key": compute_issue_key(i.type, i.slug, i.message),
                    "type": i.type,
                    "severity": i.severity,
                    "slug": i.slug,
                    "message": i.message,
...
def lint_all(*, include_stale: bool = True) -> LintReport:
    ...
    # P1-12b: 过滤已忽略的 issue（持久化在 lint_ignores 表）
    ignored_keys = _load_ignored_keys()
    if ignored_keys and report.issues:
        kept: list[LintIssue] = []
        ignored_count = 0
        for issue in report.issues:
            if compute_issue_key(issue.type, issue.slug, issue.message) in ignored_keys:
                ignored_count += 1
            else:
                kept.append(issue)
        if ignored_count:
...
# ────────── P1-12b: lint issue 忽略持久化 ──────────


_LINT_DB_PATH = Path(__file__).parent.parent.parent / "data" / "wiki_lint.db"


def _get_lint_db() -> sqlite3.Connection:
    """获取 lint 忽略库连接（懒建表，复用项目既有 sqlite 惯例）"""
    _LINT_DB_PATH.parent.mkdir(parents=True, exist_ok=True)
    conn = sqlite3.connect(str(_LINT_DB_PATH))
    conn.row_factory = sqlite3.Row
    conn.execute("PRAGMA journal_mode=WAL")
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS lint_ignores (
            issue_key  TEXT PRIMARY KEY,
            type       TEXT NOT NULL,
            slug       TEXT NOT NULL,
            message    TEXT NOT NULL,
            reason     TEXT DEFAULT '',
            ignored_by TEXT DEFAULT '',
...
def ignore_issue(
    ...
    *,
    ...
    """忽略一个 lint issue（幂等：重复忽略仅更新 reason/ignored_by）"""
    conn = _get_lint_db()
    now = datetime.now(timezone.utc).isoformat()
    conn.execute(
        """
        INSERT INTO lint_ignores (issue_key, type, slug, message, reason, ignored_by, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(issue_key) DO UPDATE SET
            reason = excluded.reason,
            ignored_by = excluded.ignored_by
        """,
        (issue_key, type, slug, message, reason, ignored_by, now),
    )
    conn.commit()
    conn.close()
    logger.info(
        "wiki_lint_issue_ignored", issue_key=issue_key, slug=slug, type=type
    )
    return {"issue_key": issue_key, "ignored": True}


def unignore_issue(issue_key: str) -> bool:
    """取消忽略一个 lint issue。返回是否确实删除了一条记录"""
    conn = _get_lint_db()
    cur = conn.execute("DELETE FROM lint_ignores WHERE issue_key = ?", (issue_key,))
    conn.commit()
    deleted = cur.rowcount > 0
    conn.close()
    if deleted:
        logger.info("wiki_lint_issue_unignored", issue_key=issue_key)
    return deleted


def list_ignored_issues() -> list[dict]:
    """列出所有已忽略的 lint issue（按忽略时间倒序）"""
    conn = _get_lint_db()
    rows = conn.execute(
        "SELECT issue_key, type, slug, message, reason, ignored_by, created_at "
        "FROM lint_ignores ORDER BY created_at DESC"
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]


def _load_ignored_keys() -> set[str]:
    """加载已忽略 issue 的 key 集合（供 lint_all 过滤）"""
    conn = _get_lint_db()
