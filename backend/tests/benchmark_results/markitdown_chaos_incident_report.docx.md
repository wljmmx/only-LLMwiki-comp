...
| 2 | 慢查询日志 | SQL: SELECT \* FROM orders WHERE status=0 AND created\_at < ... 扫描 870 万行 | 无索引导致全表扫描 |
| 3 | EXPLAIN 分析 | type=ALL, rows=8723451 | 缺少复合索引(status, created\_at) |
| 4 | 应用日志 | 连接池配置 maxActive=50, 超时 30s | 连接池过小+慢查询=雪崩 |
